import { FiSearch } from "react-icons/fi";
import { CiUser } from "react-icons/ci";

const Banner = ({ handleInputChange, handleUserChange, query, bloggerQuery }) => {
    return (
        <div className="max-w-screen-2xl container mx-auto xl:px-24 md:py-20 py-14 px-4">
            {/* Heading */}
            <h1 className="text-5xl font-bold text-primary mb-3">
                Elevate Your<span className="text-green"> Blogging Experience</span> !!
            </h1>
            {/* sub-heading  */}
            <p className="text-lg text-black/70 mb-8">
                Explore a World of Writing Possibilities Awaiting You !
            </p>
            {/* Search Bar */}
            <form className="">
                <div className="flex justify-start md:flex-row flex-col md:gap-0 gap-4">
                    <div className="flex md:rounded-s-md rounded shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 md:w-1/2 w-full">
                        <input
                            type="text"
                            name="username"
                            id="username"
                            className="block flex-1 border-0 bg-transparent py-1.5 pl-8 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6"
                            placeholder="What is your best blog ?"
                            onChange={handleInputChange}
                            value={query}
                        />
                        <FiSearch className="absolute mt-2.5 ml-2 text-green" />
                    </div>

                    <div className="flex md:rounded-none rounded ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 md:w-1/3">
                        <input
                            type="text"
                            name="blogername"
                            id="blogername"
                            className="block flex-1 border-0 bg-transparent py-1.5 pl-8 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6"
                            placeholder="Who is your favourite blogger ?"
                            onChange={handleUserChange}
                            value={bloggerQuery}
                        />
                        <CiUser className="absolute mt-2.5 ml-2 text-green" />
                    </div>

                    <button
                        className="bg-green hover:bg-blue py-2 px-8 text-white md:rounded-e-md md:rounded-s-none rounded"
                    >
                        Search
                    </button>
                </div>
            </form>
        </div>
    );
};

export default Banner;
